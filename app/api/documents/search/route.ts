...
export async function GET(request: NextRequest) {
  ...
  const upstreamUrl = new URL(backendUrl("/api/documents/search"));
  for (const key of ["chat_id", "query"]) {
  ...
  const data = await upstream.json().catch(() => ({ error: "Document search failed" }));
