...
| S2 | **RESOLVED (stale, verified 2026-08-04; ringback since removed 2026-09-03)** | Audit claimed Kokoro daemon's `0.0.0.0:9237` bind needed switching to loopback. Checked live: the `0.0.0.0` bind was deliberate (comment at `kokoro_daemon.py:29-31` — ringback's docker container reached it via `host.docker.internal`, not loopback) and a `KOKORO_AUTH_TOKEN` gate (`_get_auth_token`/`_auth_ok`, `:36-46`) already closes it. No fix needed then; ringback itself is gone now, bind stays 0.0.0.0 as a general docker-consumer default | `tools-harness/kokoro_daemon.py:29-46` | n/a | confirmed via read |
| S3 | **RESOLVED (stale, verified 2026-08-04)** | Audit claimed hardcoded `<redacted-email>`/`<redacted-password>` in client React. Repo-wide grep (`.tsx`/`.ts`/`.py`, excluding `.next`/`node_modules`) — zero hits. Already removed | `lib/auth-context.tsx`, `app/auth/signin/page.tsx` | n/a | grep, zero hits |
...
| Q7 | **HIGH — open** | Golden query regression suite (`tests/golden_queries*`, runs as `task_worker` automation "Golden query regression suite") clean 100% pass through 2026-08-15, then steady climb in judge-FAILs since — orchestrator increasingly hedges/refuses instead of calling `ask_email_agent`/`ask_calendar_agent`/iMessage tools the fanout tests require ("as an AI I cannot access X", "couldn't directly check your calendar", "check other sources" instead of a real tool call). ~50% pass rate the last 3 weeks vs 100% before 08-16. Not infra/flake — verified via `get_automation_history` run-by-run, same failure shape recurring daily. Root cause not yet found; candidate causes: system-prompt drift in `harness.py`'s orchestrator prompt, or tool-calling discipline degrading under the now-default `openai/gpt-4o-mini` cloud tier (weaker than the deepseek/haiku tiers the suite passed 100% under) | `harness.py` orchestrator prompt + `ORCHESTRATOR_SYSTEM_PROMPT`, `jobs/handlers/*golden*` runner, `clients/cloud_client.py` model chain | investigation, not yet scoped | Re-run golden suite after any prompt/model change, compare pass rate against pre-08-16 baseline |
...
| W6 | Reminder with escalation | Notification first, phone call if unacknowledged after N min | `jobs/worker.py` scheduling (phone-call leg needs a new SIP/voice connector — ringback removed 2026-09-03) |
